...
def train_net_with_evaluation(X_train, y_train, X_test, y_test, data_iter=None, test_iter=None, net=None,
    ...
    if draw == 'loss':
        for epoch in range(num_epochs):
            train_loss_sum, train_acc_sum, test_acc_sum = 0.0, 0.0, 0.0
            for X, y in data_iter:
                y_hat = net(X)  # 输入的X经过net所计算出的值
                loss_value = loss(y_hat, y)
                optimizer.zero_grad()  # 清除上一次的梯度值
        ...
        plt.show()
        # draw_Loss_or_Accuracy([train_acc_values, test_acc_values], num_epochs, show_interval, content='acc')
