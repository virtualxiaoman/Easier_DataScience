...
你也可以根据我写在下面的[三、项目功能](#jump_3)来查看一些具体的用法，
...
`[·]`代表正在进行中,`[x]`代表废弃或用处不大的项目,`[√]`代表基本完成的项目
...
├── easier_nn                   # [·] 使用神经网络进行计算机视觉(CV)和自然语言处理(NLP)等任务
│   ├── __init__.py             # [√] 初始化
│   ├── calculaye_shape.py      # [√] 计算数据的shape，可以将网络传入，然后查看每一层的shape
...
def fill_missing_values(self, fill_type='mean'):
    ...
    pass
```
其中`Warning`是对使用此函数时的一些警告，`使用方法`是对函数的使用方法，`Tips`是对函数的一些小提示，`Bug`是还未修复的一些问题，`todo`是还未完成的任务。
