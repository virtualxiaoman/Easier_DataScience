...
print("星级=4的", end='')
rd.DescDF(df_4).describe_df(stats_detailed=False, show_nan=False)
print("星级=5的", end='')
rd.DescDF(df_5).describe_df(stats_detailed=False, show_nan=False)


print("----------处理异常值与缺失值----------")
# 缺失值处理(因为数值型数据无缺失值，这里其实可以不处理)
# DescDF.fill_missing_values(fill_type='mean')  # 均值填补
